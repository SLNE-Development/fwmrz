...
<div class="bg-zinc-900 border-b border-zinc-800 pt-12 pb-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p class="text-zinc-400 max-w-2xl">
            Auf unserer Internetseite berichten wir ausführlich (inkl. Bild- und Videomaterial) über unsere Einsätze.
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <p class="text-sm text-zinc-500 mb-8">{{ number_format($commitments->total()) }} Einsätze gefunden</p>
    @endif
           ...
           class="group bg-zinc-900 border border-zinc-800 hover:border-red-700/60 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-red-950/20 flex flex-col">
            <div class="p-5 flex flex-col flex-1">
                <div class="flex items-start justify-between gap-3 mb-3">
                    @if($commitment->type)
                    <span class="shrink-0 inline-flex items-center bg-red-900/40 border border-red-800/40 text-red-400 text-xs font-bold px-2 py-0.5 rounded uppercase tracking-wide">
                        {{ $commitment->type->short }}
                    </span>
                    @endif
                    <span class="text-xs text-zinc-500 shrink-0">{{ $commitment->start->format('d.m.Y') }}</span>
                </div>
                <h2 class="text-base font-bold text-white group-hover:text-red-400 transition-colors mb-1 line-clamp-2">
                    {{ $commitment->title }}
                </h2>
                @if($commitment->type)
                ...
                @endif
                @if($commitment->body)
                <p class="text-sm text-zinc-400 line-clamp-3 flex-1">{{ Str::limit(strip_tags($commitment->body), 120) }}</p>
                @endif
                <div class="mt-4 flex items-center justify-between">
                    <span class="text-xs text-zinc-600">{{ $commitment->start->format('H:i') }} Uhr</span>
                    <span class="text-xs text-red-400 font-medium group-hover:gap-2 flex items-center gap-1.5 transition-all">
                        Mehr erfahren
