...
@include("pages.einsaetze.ld-show")
@endsection
...
<div class="bg-zinc-900 border-b border-zinc-800 pt-12 pb-10">
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    <div
        class="flex items-center gap-2 text-xs text-zinc-500 mb-4 uppercase tracking-wider flex-wrap">
      <a href="{{ route('home') }}" class="hover:text-red-400 transition-colors">Startseite</a>
      <span>›</span>
      <a href="{{ route('einsaetze.index') }}"
         class="hover:text-red-400 transition-colors">Einsätze</a>
      <span>›</span>
    ...
    <div class="flex flex-wrap items-center gap-3 mb-4">
      @if($commitment->type)
      <span
          class="inline-flex items-center bg-red-900/50 border border-red-800/50 text-red-400 text-xs font-bold px-3 py-1.5 rounded-lg uppercase tracking-wide">
    ...
    <div class="flex flex-wrap gap-2 mt-4">
      ...
      <span
          class="inline-block bg-zinc-800 border border-zinc-700 text-zinc-300 text-xs px-2.5 py-1 rounded-lg">
                {{ $station->name }}
            </span>
      @endforeach
    </div>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
  ...
  <div class="mb-12" x-data="{
    ...
    }" @keydown.escape.window="open = false" @keydown.arrow-left.window="if(open) prev()"
    ...
    <div x-show="open" x-cloak
      ...
      <button @click="prev()"
              class="absolute left-4 top-1/2 -translate-y-1/2 text-white/70 hover:text-white bg-zinc-800/60 hover:bg-zinc-700 rounded-full p-2 transition">
        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/>
        </svg>
      </button>
      <img :src="current" class="max-h-[90vh] max-w-[90vw] object-contain rounded-xl shadow-2xl"
           alt="">
      <button @click="next()"
              class="absolute right-4 top-1/2 -translate-y-1/2 text-white/70 hover:text-white bg-zinc-800/60 hover:bg-zinc-700 rounded-full p-2 transition">
        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
        </svg>
      </button>
      <button @click="open = false"
              class="absolute top-4 right-4 text-white/70 hover:text-white bg-zinc-800/60 hover:bg-zinc-700 rounded-full p-2 transition">
